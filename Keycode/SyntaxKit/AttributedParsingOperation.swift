...
public struct Diff {
    ...
    func representsChangesfromOldString(_ oldString: NSString, toNewString newStr: NSString) -> Bool {
        if self.range.length == 0 {
            if !Diff.stringChangeIsCompatible(newStr, isInsertion: true, changedRange: NSRange(location: self.range.location, length: (self.change as NSString).length), oldString: oldString) {
                return false
            }
            if newStr.substring(with: NSRange(location: self.range.location, length: (self.change as NSString).length)) != self.change {
                return false
            }
        } else {
            if !Diff.stringChangeIsCompatible(newStr, isInsertion: false, changedRange: self.range, oldString: oldString) {
                return false
            }
        }
    ...
    ///             the new string
    static func stringChangeIsCompatible(_ newString: NSString, isInsertion insertion: Bool, changedRange range: NSRange, oldString: NSString) -> Bool {
...
public class AttributedParsingOperation: Operation {
    ...
    public init(string: String, previousOperation: AttributedParsingOperation, changeIsInsertion insertion: Bool, changedRange range: NSRange, callback: OperationCallback? = nil) {
        ...
        let diff: Diff
        if insertion {
            diff = Diff(change: s.substring(with: range), range: NSRange(location: range.location, length: 0))
        } else {
            diff = Diff(change: "", range: range)
        }

        if diff.representsChangesfromOldString(previousOperation.scopedStringResult.underlyingString as NSString, toNewString: string as NSString) {
            self.diff = diff
            self.range = outdatedRangeForChangeInString(string as NSString, changeIsInsertion: insertion, changedRange: range)
        }
    }
    ...
    func outdatedRangeForChangeInString(_ newString: NSString, changeIsInsertion insertion: Bool, changedRange range: NSRange) -> NSRange? {
        if !Diff.stringChangeIsCompatible(newString, isInsertion: insertion, changedRange: range, oldString: scopedStringResult.underlyingString as NSString) {
            return nil
        }
        ...
        if insertion {
            potentialNewString.insertString(newString.substring(with: range), atIndex: range.location)
        ...
        if potentialNewString.underlyingString != newString as String {
            return nil
        }
